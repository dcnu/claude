...
5. If `auto` suggestions exist, present them for batch selection using AskUserQuestion:
   ...
   - Classifications for "auto":
     - Only needs delimiter change (`_` → `-`)
     - Only needs case normalization
     - Only needs date format conversion
     - All components clearly detected
...
3. If user accepts, for each file in `needsReview`:
   ...
   - Analyze content to determine:
     - Source (person, organization, or omit if unclear)
     - Title (document subject, or omit if unclear)
     - Date (extract from content or metadata)
...
4. Generate LLM suggestions following these rules:
   ...
   - If only date is known: `Title-date.ext` or just keep original
   - If nothing clear: skip the file entirely
...
- Allows `--new-only` to skip already-processed files
...
The script recognizes these common abbreviations:
- Consulting: `BCG`, `McKinsey`, `Bain`, `Deloitte`, `PwC`, `EY`, `KPMG`
- Government: `FBI`, `CIA`, `NASA`, `EPA`, `FDA`, `SEC`, `FTC`, `IRS`
- Finance: `JPM`, `GS`, `MS`, `Citi`, `BoA`, `HSBC`
- Tech: `MSFT`, `GOOG`, `AAPL`, `META`, `AMZN`, `IBM`

## Examples

### Input
```
BCG Report Q1 2024.pdf
john_smith_resume_2024-01-15.docx
meeting-notes-jan-2024.txt
random-scan.pdf
IMG_4521.jpg
```
...
### Output (Phase 1 - Auto)
```
Auto-Renames (3 files):
[x] BCG Report Q1 2024.pdf → BCG-Report-2024-Q1.pdf
[x] john_smith_resume_2024-01-15.docx → SmithJohn-Resume-240115.docx
[x] meeting-notes-jan-2024.txt → MeetingNotes-2024-January.txt

3 selected. Rename? [Y/n]
```

### Output (Phase 2 - Needs Review)
```
...
```
LLM Suggestions:
[x] random-scan.pdf → TaxReturn-2024.pdf
[ ] IMG_4521.jpg → (skipped - no clear metadata)

1 selected. Rename? [Y/n]
```
